...
ImageCapturer::ImageCapturer(CameraHal &camera, ActuatorHal &actuator, Mux &mux,
    ...
    : camera_(camera), actuator_(actuator), mux_(mux), camera_type_(cam_type),
      format_(format), controls_(default_controls),
      default_output_(default_output), configured_(false) {
...
bool ImageCapturer::start() {
  ...
  if (!configured_) {
    if (!mux_.select(camera_type_))
      return false;
    if (!camera_.set_format(format_))
      return false;
    if (!camera_.set_controls(controls_))
      return false;
    configured_ = true;
  }
